...
import { Business, getBusiness, firstBusinessId, ensureSchema } from "./db";
...
export async function getActiveBusinessId(): Promise<number> {
  ...
  if (Number.isInteger(id) && (await getBusiness(id))) return id;
...
export async function getActiveBusiness(): Promise<Business> {
  ...
  return (await getBusiness(id)) ?? (await getBusiness(await firstBusinessId()))!;
}
